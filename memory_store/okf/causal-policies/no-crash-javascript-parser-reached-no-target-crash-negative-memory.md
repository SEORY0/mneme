---
type: causal-policy
title: No Crash Javascript Parser Reached No Target Crash Negative Memory
description: Negative memory for no_crash with parser_reached_no_target_crash on javascript inputs.
...
success_count: 0
confidence: medium
tags: [no-crash, parser-reached-no-target-crash, javascript, heap-buffer-overflow-read, construct, negative-memory]
match_keys: [no-crash, parser-reached-no-target-crash, javascript, heap-buffer-overflow-read, negative-memory]
allowed_scopes: [generate]
forbidden_fields: [raw_poc_bytes, task_id, exact_offset, checksum, submit_metadata]
evidence_level: medium
train_only: true
---
# No Crash Javascript Parser Reached No Target Crash Negative Memory
...
## Failure Shape
Script-only lexer probes covered malformed numeric literals, comments, strings, keywords,
and regular-expression contexts, but all completed without sanitizer output. The remaining
gap is the specific regexp-literal parser state that makes the lexer look past the script
buffer while still being parsed as a regexp rather than division or an early syntax error.

## Policy
Treat `parser_reached_no_target_crash` after `no_crash` on `javascript` as evidence that the candidate preserved or missed
