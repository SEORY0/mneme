---
type: causal-policy
title: No Crash Php Serialized Value Sink Not Reached Negative Memory
description: Negative memory for no_crash with sink_not_reached on php-serialized-value inputs.
failure_class: no_crash
verifier_signal: sink_not_reached
candidate_family: construct
input_format: php-serialized-value
harness_convention: libfuzzer
vuln_class: use-after-scope
access_scope: generate
success_count: 0
confidence: medium
tags: [no-crash, sink-not-reached, php-serialized-value, use-after-scope, construct, negative-memory]
match_keys: [no-crash, sink-not-reached, php-serialized-value, use-after-scope, negative-memory]
allowed_scopes: [generate]
forbidden_fields: [raw_poc_bytes, task_id, exact_offset, checksum, submit_metadata]
evidence_level: medium
train_only: true
---
# No Crash Php Serialized Value Sink Not Reached Negative Memory

- key: `no_crash x sink_not_reached`
- outcome: persistent failure / basin to avoid
- success_count: 0
- failure_count: 1

## Failure Shape
The harness takes raw serialized values. In-tree storage corpus entries and custom
reference-heavy storage layouts parsed without triggering the object-lifetime violation. The
unresolved gate is the exact serialized storage member graph that leaves an attachment or
object reference scoped out while still reachable during destruction.

## Policy
Treat `sink_not_reached` after `no_crash` on `php-serialized-value` as evidence that the candidate preserved or missed
