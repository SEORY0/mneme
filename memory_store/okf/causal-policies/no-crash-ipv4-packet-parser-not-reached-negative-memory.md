---
type: causal-policy
title: No Crash Ipv4 Packet Parser Not Reached Negative Memory
description: Negative memory for no_crash with parser_not_reached on ipv4-packet inputs.
failure_class: no_crash
verifier_signal: parser_not_reached
candidate_family: construct
input_format: ipv4-packet
harness_convention: libfuzzer
vuln_class: out-of-bounds-read
access_scope: generate
success_count: 0
confidence: medium
tags: [no-crash, parser-not-reached, ipv4-packet, out-of-bounds-read, construct, negative-memory]
match_keys: [no-crash, parser-not-reached, ipv4-packet, out-of-bounds-read, negative-memory]
allowed_scopes: [generate]
forbidden_fields: [raw_poc_bytes, task_id, exact_offset, checksum, submit_metadata]
evidence_level: medium
train_only: true
---
# No Crash Ipv4 Packet Parser Not Reached Negative Memory
...
## Failure Shape
The harness is compiled for the IP dissector and disables UDP, so direct O-RAN/eCPRI
payloads did not reach the ORAN extension parser. Source inspection shows the target
invariant is extension bundle accounting with a reserved zero bundle size followed by use of
the last bundle entry; the unresolved gate is an IP-level encapsulation that reaches the
O-RAN C-plane dissector under this fuzzshark configuration.

## Policy
Treat `parser_not_reached` after `no_crash` on `ipv4-packet` as evidence that the candidate preserved or missed
