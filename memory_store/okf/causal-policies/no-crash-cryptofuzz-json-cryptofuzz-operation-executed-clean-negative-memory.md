---
type: causal-policy
title: No Crash Cryptofuzz Json Cryptofuzz Operation Executed Clean Negative Memory
description: Negative memory for no_crash with cryptofuzz_operation_executed_clean on cryptofuzz-json inputs.
failure_class: no_crash
verifier_signal: cryptofuzz_operation_executed_clean
candidate_family: construct
input_format: cryptofuzz-json
harness_convention: libfuzzer
vuln_class: api-initialization-error
access_scope: generate
success_count: 0
confidence: medium
tags: [no-crash, cryptofuzz-operation-executed-clean, cryptofuzz-json, api-initialization-error, construct, negative-memory]
match_keys: [no-crash, cryptofuzz-operation-executed-clean, cryptofuzz-json, api-initialization-error, negative-memory]
allowed_scopes: [generate]
forbidden_fields: [raw_poc_bytes, task_id, exact_offset, checksum, submit_metadata]
evidence_level: medium
train_only: true
---
# No Crash Cryptofuzz Json Cryptofuzz Operation Executed Clean Negative Memory

- key: `no_crash x cryptofuzz_operation_executed_clean`
- outcome: persistent failure / basin to avoid
- success_count: 0
- failure_count: 1

## Failure Shape
The candidate used an HMAC-signing style operation with an empty key, but the cryptofuzz
OpenSSL-API target executed cleanly. The remaining gap is the exact cryptofuzz operation
schema that maps to the wolfCrypt OpenSSL HMAC context creation path.

## Policy
Treat `cryptofuzz_operation_executed_clean` after `no_crash` on `cryptofuzz-json` as evidence that the candidate preserved or missed
