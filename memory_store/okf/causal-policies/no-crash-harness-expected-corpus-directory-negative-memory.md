---
type: causal-policy
title: No Crash Harness Expected Corpus Directory Negative Memory
description: Negative memory for no_crash with verifier signal harness_expected_corpus_directory.
failure_class: no_crash
verifier_signal: harness_expected_corpus_directory
candidate_family: seed-mutate
input_format: any
harness_convention: any
access_scope: generate
success_count: 0
confidence: medium
tags: [no-crash, harness-expected-corpus-directory, negative_memory]
match_keys: [no-crash, harness-expected-corpus-directory, negative_memory]
allowed_scopes: [generate]
forbidden_fields: [raw_poc_bytes, task_id, exact_offset, checksum, submit_metadata]
evidence_level: medium
train_only: true
---
# No Crash Harness Expected Corpus Directory Negative Memory

## Policy
Treat this verifier signal as negative memory for the attempted candidate family. Preserve any parser reachability it proved, but do not keep mutating the same field basin unless the next verification changes the signal.

## Round 4 Reinforcement
- key: `no_crash x harness_expected_corpus_directory`
- outcome: persistent failure basin
- support_count: 1
- candidate_families: seed-mutate
- observed_formats: jpeg

### Procedure
Treat this as an envelope or harness-shape failure. Rebuild the carrier around the exact fuzzer input contract, confirm parser reachability, then add one target invariant.

### Diagnosed Dead Ends
- The transform fuzzer target requires a JPEG that reaches the final transform without metadata-copy suppression or destination reallocation. The available wrapper treated the single uploaded PoC path as a corpus directory requirement, so seed-derived JPEG files were not executed through the intended transform operation by the runner.

### Negative Memory
Do not repeat the same carrier plus broad mutation after this signal. Do not promote this basin into a recovery until a later verifier-confirmed candidate flips the official gate.
