---
type: causal-policy
title: No Crash Ipv4 Tcp Tls Parser Not Reached Negative Memory
description: Negative memory for no_crash with parser_not_reached on ipv4-tcp-tls inputs.
...
success_count: 0
confidence: medium
tags: [no-crash, parser-not-reached, ipv4-tcp-tls, use-of-uninitialized-value, construct, negative-memory]
match_keys: [no-crash, parser-not-reached, ipv4-tcp-tls, use-of-uninitialized-value, negative-memory]
allowed_scopes: [generate]
forbidden_fields: [raw_poc_bytes, task_id, exact_offset, checksum, submit_metadata]
evidence_level: medium
train_only: true
---
# No Crash Ipv4 Tcp Tls Parser Not Reached Negative Memory
...
## Failure Shape
A whole IPv4/TCP packet carrying a TLS handshake did not reach the target TLS risk path.
Prior memory also indicates raw TLS records and minimal TCP wrapping are insufficient; the
missing gate is likely nDPI flow state or packet sequencing that dispatches into ClientHello
or ServerHello processing before the malformed extension data is copied into risk strings.

## Policy
Treat `parser_not_reached` after `no_crash` on `ipv4-tcp-tls` as evidence that the candidate preserved or missed
