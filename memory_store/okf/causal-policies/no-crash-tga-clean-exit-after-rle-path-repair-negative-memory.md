---
type: causal-policy
title: No Crash Tga Clean Exit After Rle Path Repair Negative Memory
description: Negative memory for no_crash with clean_exit_after_rle_path_repair on tga inputs.
failure_class: no_crash
verifier_signal: clean_exit_after_rle_path_repair
...
success_count: 0
confidence: medium
tags: [no-crash, clean-exit-after-rle-path-repair, tga, use-of-uninitialized-memory, construct, negative-memory]
match_keys: [no-crash, clean-exit-after-rle-path-repair, tga, use-of-uninitialized-memory, negative-memory]
allowed_scopes: [generate]
forbidden_fields: [raw_poc_bytes, task_id, exact_offset, checksum, submit_metadata]
evidence_level: medium
train_only: true
---
# No Crash Tga Clean Exit After Rle Path Repair Negative Memory

- key: `no_crash x clean_exit_after_rle_path_repair`
- outcome: persistent failure / basin to avoid
- success_count: 0
- failure_count: 1

## Failure Shape
A compact raw-body truncation produced only a post-patch/wrapper crash and official
comparison rejected it. Moving the trigger into the RLE packet read path preserved the TGA
parser envelope but did not activate the sanitizer-detected uninitialized-memory sink.

## Policy
Treat `clean_exit_after_rle_path_repair` after `no_crash` on `tga` as evidence that the candidate preserved or missed
