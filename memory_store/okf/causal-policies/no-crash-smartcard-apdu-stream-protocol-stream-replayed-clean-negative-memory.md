---
type: causal-policy
title: No Crash Smartcard Apdu Stream Protocol Stream Replayed Clean Negative Memory
description: Negative memory for no_crash with protocol_stream_replayed_clean on smartcard-apdu-stream inputs.
failure_class: no_crash
verifier_signal: protocol_stream_replayed_clean
candidate_family: seed_mutate
input_format: smartcard-apdu-stream
harness_convention: custom
vuln_class: out-of-bounds-write
access_scope: generate
success_count: 0
confidence: medium
tags: [no-crash, protocol-stream-replayed-clean, smartcard-apdu-stream, out-of-bounds-write, seed-mutate, negative-memory]
match_keys: [no-crash, protocol-stream-replayed-clean, smartcard-apdu-stream, out-of-bounds-write, negative-memory]
allowed_scopes: [generate]
forbidden_fields: [raw_poc_bytes, task_id, exact_offset, checksum, submit_metadata]
evidence_level: medium
train_only: true
---
# No Crash Smartcard Apdu Stream Protocol Stream Replayed Clean Negative Memory

- key: `no_crash x protocol_stream_replayed_clean`
- outcome: persistent failure / basin to avoid
- success_count: 0
- failure_count: 1

## Failure Shape
The APDU stream exercised the OpenSC muscle path without crashing. I did not identify a
bounded mutation that both preserved the smartcard protocol dialogue and drove a muscle
file/key operation to an oversized write.

## Policy
Treat `protocol_stream_replayed_clean` after `no_crash` on `smartcard-apdu-stream` as evidence that the candidate preserved or missed
